...
struct HashTable{
	...
	//add your code below
        for (int i=0;i<user_name.length();i++){
            sum += user_name[i];; //sum of ascii values of characters in user_name
        }
	...
	//add an if condition to complete the code here
        if (empty){
            password[hash] = user_password; // insert password into the hash table
        }
        else{
	...
	//add an if condition to complete the code here
        if (empty){
            cout<<"No user found\n"; // if the slot is empty then no user is found
        }
        else{
            cout<<"Password for "<<user_name<<" is "<<password[hash]<<endl; // print the password for the user
        }
